...
async function chooseMetroPort() {
  if (await isPortAvailable(metroPort)) return;

  const requestedPort = metroPort;
  for (let candidate = requestedPort + 1; candidate <= requestedPort + 20; candidate++) {
    if (await isPortAvailable(candidate)) {
      metroPort = candidate;
      console.log(`Metro port ${requestedPort} is busy; using ${metroPort}`);
      return;
    }
  }

  throw new Error(`No free Metro port found near ${requestedPort}`);
}
